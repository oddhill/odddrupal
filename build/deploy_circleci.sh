...
CURRENT_DOCROOT=$BASE_PATH/portal-sedermera-circleci
NEW_DOCROOT=$BASE_PATH/portal-sedermera-circleci-new
...
ssh root@oddserver.se /bin/bash << EOF
  ...
  # Copy files from the current docroot to the new one, if the current directory
  # exists, which won't be the case if this is a fresh deploy.
